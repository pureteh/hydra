...
module Hydra.Chain.Direct.Explorer.Server where
...
import Hydra.Chain.Direct.Explorer (ChainEvent, ExplorerConfig (..), runChainExplorer)
...
httpApp req send =
  case (requestMethod req, pathInfo req) of
    ...
    -- FIXME: this is very dangerous, just for quick testing
    ("GET", file) -> send $ handleFile $ concat $ intercalate "/" file
    (_, _) ->
      send handleNotFound
